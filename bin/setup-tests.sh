...
if [ ! -L "${WP_DIR}/wp-content/plugins/test-plugin" ]; then
  ln -s "${TEST_DIR}/data/test-plugin" "${WP_DIR}/wp-content/plugins/test-plugin"
fi

wp plugin activate --path="${WP_DIR}" test-plugin
